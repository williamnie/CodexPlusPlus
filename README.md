...
- 远程桌面：启动后输出本机和局域网访问地址，可在浏览器中查看会话、发送消息、创建新对话，并支持任务完成提醒。
...
2. 启动本地 helper 服务，用于健康检查、设置、导出、移动、删除和远程桌面等操作。
...
运行 `python -m codex_session_delete launch` 后，终端会打印远程桌面访问地址：
...
打开 `Local` 地址可在本机浏览器中使用；打开 `LAN` 地址可在同一局域网的手机或其他电脑上使用。首次访问需要 URL 中的 `token`，前端会保存 token 并清理地址栏，后续接口请求通过 `X-Web-Token` 请求头鉴权。

远程桌面支持：

- 按项目查看会话列表，并打开历史会话。
- 同步桌面端当前对话状态，查看消息和流式输出。
- 发送新消息、创建新对话，可选择继承项目目录。
- 桌面端和 Web 端查看同一线程时自动合并最新消息。
- 任务完成提醒：可开启浏览器提示音、Toast 和系统通知。

注意事项：

- helper 服务默认监听 `0.0.0.0:<helper-port>`；局域网访问需要系统防火墙允许该端口。
- 访问 token 只在本次启动生成，请不要公开分享带 token 的 URL。
- 如需自定义端口，可启动时传入 `--helper-port`。
